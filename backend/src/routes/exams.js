    ...
    db.all(sql, params, (err, rows) => {
      ...
      rows.forEach(r => {
        if (r.status_code === 2 && r.started_at && r.duration_min > 0) {
          ...
          // Only auto-finish if we have strictly passed the end time
          if (now > end) {
...
router.get("/:id/download-subject", authenticateToken, (req, res) => {
  const examId = req.params.id;
  db.get("SELECT sujet_path FROM examen WHERE id = ?", [examId], (err, row) => {
    if (err || !row?.sujet_path) {
      console.error("Sujet download error:", err?.message || "No sujet_path found");
      return res.status(404).json({ error: "Sujet introuvable" });
    }

    // sujet_path is already relative to uploads/ (e.g., "exams/17/subject.pdf")
    const filePath = path.join(__dirname, "../../uploads", row.sujet_path);

    console.log("Attempting to download subject from:", filePath);
    ...
    res.download(filePath, `sujet_examen_${examId}.pdf`, (err) => {
      if (err) {
...
router.post("/:id/finish", authenticateToken, (req, res) => {
  ...
  let sql = "UPDATE examen SET status_code=4, status='finished', finished_at=? WHERE id=?";
  let params = [now, req.params.id];
...
router.get("/:id/export", authenticateToken, (req, res) => {
  ...
  db.get("SELECT * FROM examen WHERE id=?", [examId], (err, exam) => {
    if (err || !exam) return res.status(404).json({ error: "Examen introuvable" });

    // Check ownership if professor
    if (req.user.role === 'professor' && exam.professor_id !== req.user.id) {
    ...
    db.all(sql, [examId], (err, rows) => {
      ...
      rows.forEach(row => {
        ...
        // Don't create folder yet - wait to see if we have valid files
        let hasFiles = false;
            ...
            if (Array.isArray(files) && files.length > 0) {
              ...
              files.forEach(f => {
                ...
                if (fs.existsSync(src)) {
                  // Ensure dir exists only when we find the first valid file
                  if (!hasFiles) {
      ...
      ps.on('close', (code) => {
        ...
        if (!fs.existsSync(zipPath)) {
          ...
          return res.status(500).json({ error: "Le fichier ZIP n'a pas été créé" });
        }
