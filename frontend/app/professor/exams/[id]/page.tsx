...
export default function ProfessorMonitor() {
    ...
    useEffect(() => {
        ...
        socket.on("student-connected", (p) => {
            ...
            addLog(`Connexion: Étudiant ${p.matricule || p.studentId}`, 'success');
        });

        socket.on("student-disconnected", (p) => {
            updateStudent(p.studentId, { status: "offline", lastSeen: p.at });
            addLog(`Déconnexion: Étudiant ${p.matricule || p.studentId}`, 'warn');
        });
        ...
        socket.on("cheat-alert", (p) => {
            ...
            toast(`⚠️ Fraude potentielle: ${p.matricule || p.studentId}`);
            addLog(`Alerte fraude: Étudiant ${p.matricule || p.studentId} - ${detailsAR}`, 'warn');
        });
        ...
        socket.on("finalize-exam", (p: any) => {
            ...
            toast(`🎓 L'étudiant ${p.matricule || sid} a terminé l'examen`);
            addLog(`Examen terminé: Étudiant ${p.matricule || sid}`, 'info');
        });
        ...
        socket.on("file-submitted", (p) => {
            ...
            const fileName = p.files && p.files.length > 0 ? p.files[0].name : "fichier";
            toast(`📥 Nouveau fichier reçu de ${p.matricule || p.studentId}`);
            addLog(`Fichier reçu: ${fileName} (Étudiant: ${p.matricule || p.studentId})`, 'info');
        });
        ...
        socket.on("exam-ended", () => {
            setIsEnded(true);
            setTimerStr("انتهى");
            addLog("Temps d'examen écoulé", 'warn');
        });
    ...
    function translateCheat(type: string, details: string) {
        if (type === "TAB_SWITCH") return "Changement d'onglet";
        if (type === "FOCUS_LOST") return "Perte de focus";
        if (type === "COPY_PASTE_ATTEMPT") return "Tentative copier-coller";
        if (type === "DEV_TOOLS_ATTEMPT") return "Outils développeur";
        return details;
    }
                ...
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-indigo-50/50">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                            📂 Fichiers de l'étudiant <span className="text-indigo-600">{matricule || studentId}</span>
                        </h2>
                        <p className="text-xs text-slate-500 mt-1">Identifiant: {studentId}</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar space-y-3">
                    {submittedFiles.length === 0 ? (
                        <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
                            ...
                            <p className="text-slate-400 font-bold mt-2">Aucun fichier soumis</p>
                        </div>
                    ) : (
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <div className="flex gap-2">
                                            <span className="text-[10px] text-slate-400 bg-white px-1.5 py-0.5 rounded border border-slate-100">Fichier soumis</span>
                                            {file.exists === false && (
                                                <span className="text-[10px] text-red-600 bg-red-100 px-1.5 py-0.5 rounded font-bold">Fichier supprimé du serveur</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
